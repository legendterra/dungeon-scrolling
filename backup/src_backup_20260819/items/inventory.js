...
(function (DS) {
  ...
  function create() {
    return {
      equipped: [Loot.startingWeapon(), null],
      active: 0,
      bag: [],
      coins: 0,
      shards: 0,
  ...
  function bestItem(inv) {
    let best = null;
    const all = inv.equipped.concat(inv.bag).concat(
      DS.Armor.SLOTS.map(function (slot) { return inv.armor[slot]; }));

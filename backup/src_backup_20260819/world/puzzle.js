...
(function (DS) {
  ...
  function generateBarrier(g, level) {
    ...
    for (let i = 0; i < order.length; i++) {
      ...
      if (kind === 'keygate') {
        const warden = DS.Enemies.create(g, (baseTx + 8) * T, floorY - T,
          g.rng.weighted(DS.Enemies.spawnTable(g.depth)), 'elite');
  ...
  function updateBarrier(g, puzzle) {
    ...
    // Last-resort valve, and only while the player is actually stuck at the gate.
    if (!solved && g.player && !g.player.dead) {
    ...
    // Only the plates can be un-solved by walking away.
    if (!solved && puzzle.kind === 'plates') gate.open = false;

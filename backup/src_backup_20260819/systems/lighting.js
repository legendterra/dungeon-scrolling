...
(function (DS) {
  ...
  function ensure() {
    if (layer) return;
    layer = DS.Art.makeCanvas(C.W, C.H);
  ...
  function collect(g) {
    ...
    for (let i = 0; i < g.map.decor.length; i++) {
      ...
      if (d.kind === 'torch') {
        ...
        add(d.x + 4, d.y + 4, 44 * flicker, 0.95, biome.light);
  ...
  function render(g) {
    ...
    const R = DS.R;
    const darkness = DS.Modifiers.darknessFor(g);
    ...
    for (let i = 0; i < lights.length; i++) {
      ...
      grd.addColorStop(0, tint(l.c, 0.045 * l.s));
      grd.addColorStop(0.6, tint(l.c, 0.015 * l.s));
      grd.addColorStop(1, 'rgba(0,0,0,0)');
      cx.fillStyle = grd;

...
(function (DS) {
  ...
  function create(g, x, y, kind, rank) {
    ...
    const w = cfg.w * mult.size;
    const h = cfg.h * mult.size;
  ...
  function update(g, e) {
    ...
    const sees = player && !player.dead && dist < e.cfg.sight;
    ...
    if (!e.flying && !e.cfg.rooted) Phys.step(e, g.map);
  ...
  function draw(g, e) {
    ...
    const pack = e.tier === 'miniboss' ? root.mini[e.cfg.sprite]
    ...
    if (e.hurtFlash > 0) {
      R.spr(DS.Art.silhouette(sprite, '#ffffff'), e.x, e.y);
    } else if (tint) {
      // Whatever status is loudest colours the body, so you can read the board.
      R.spr(sprite, e.x, e.y);
      R.sprAlpha(DS.Art.silhouette(sprite, tint), e.x, e.y,
      ...
      R.spr(sprite, e.x, e.y - (e.kind === 'slime' ? 0 : Math.round(ratio * 2)));
      if (blink) R.sprAlpha(DS.Art.silhouette(sprite, '#c0303c'), e.x, e.y, 0.55);
    } else {
      R.spr(sprite, e.x, e.y);
    }
  ...
  function drawTelegraph(g, e, R) {
    if (e.attackState === 'wind') {
      ...
      if (e.kind === 'zombie') {
        ...
        R.rect(box.x, box.y + box.h - 2, box.w * ratio, 2, 'rgba(192,48,60,0.55)');
        ...
        // Squash marker on the ground where it will land.
        const landX = cx + e.facing * 34;
        R.rect(landX - 6 * ratio, e.y + e.h - 1, 12 * ratio, 1, 'rgba(192,48,60,0.5)');
      }
    } else if (e.attackState === 'strike' && e.kind === 'zombie') {
      const box = meleeBox(e);
      R.rect(box.x, box.y, box.w, box.h, 'rgba(255,255,255,0.10)');
    }
  }

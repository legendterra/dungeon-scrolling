...
(function (DS) {
  ...
  function update(g, p) {
    ...
    if (p.miniLeft < (p.miniMax || MINI_CHARGES)) {
      ...
      if (p.miniTimer <= 0) {
        ...
        DS.FX.ring(Ent.centerX(p), Ent.centerY(p), 6, '#5cbf62', 0.9);
      }
    }
  ...
  function move(g, p) {
    ...
    const speed = p.stats.moveSpeed * (p.charging ? 0.45 : 1);
  ...
  function startMiniDash(g, p) {
    ...
    DS.Audio.play('dash');
    DS.FX.ring(Ent.centerX(p), Ent.centerY(p), 6, '#5cbf62', 1.2);
  }
  ...
  function drawSwingEffect(g, p, item, base, pose, handX, handY, dir, flip) {
    ...
    if (anim.style === 'arc' || anim.style === 'chop') {
      ...
      const reach = p.pending ? p.pending.reach : 18;
      const y = handY - 1;
      R.line(handX, y, handX + dir * reach * 0.8, y,
             'rgba(255,255,255,' + fade.toFixed(2) + ')', anim.style === 'thrust' ? 2 : 1);
      R.line(handX, y - 3, handX + dir * reach * 0.5, y - 3,
             'rgba(168,228,255,' + (fade * 0.6).toFixed(2) + ')', 1);
    } else if (anim.style === 'draw') {
      drawBowString(R, pose, handX, handY, dir);
    } else if (anim.style === 'cast') {
      const color = item.element ? W.ELEMENTS[item.element].color : '#c86ee0';
      const tipX = handX + dir * 4, tipY = handY - 11;
      R.arc(tipX, tipY, 5 + pose.t * 8, -1.4, 1.4, withAlpha(color, fade), 2, flip);
    }
  }
  ...
  function drawStaffTip(R, item, pose, handX, handY, dir) {
    const color = item.element ? W.ELEMENTS[item.element].color : '#c86ee0';
    const size = 3 + pose.ratio * 4;
    const tipX = handX + dir * 3, tipY = handY - 12;
    DS.Map.glow(R, tipX, tipY, 6 + pose.ratio * 12, withAlpha(color, 0.25 + pose.ratio * 0.3));
    R.rect(tipX - size / 2, tipY - size / 2, size, size, color);
  }

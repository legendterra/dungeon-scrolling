...
(function (DS) {
  ...
  // Extra shapes stamped over the head for the bulkier and cute materials.
  const CRESTS = {
    medium: ['...LLLLLL...', '..LmmmmmmL..'],
    heavy:  ['..L......L..', '.LLmmmmmmLL.', '..LmmmmmmL..'],
    grove:  ['...L..L.....', '..LmL..LmL..', '..LmmmmmmL..'],
    bear:   ['..L....L....', '.LmL..LmL...', '..LmmmmmmL..'],
    horns:  ['.L......L...', '..L....L....', '..LmmmmmmL..'],
    plume:  ['....L.......', '...LLm......', '..LmmmmmmL..'],
    crown:  ['.L..L..L....', '.LmLmLmL....', '..LLLLLL....'],
    astral: ['..L....L....', '.LmL..LmL...', '.LLLLLLLL...'],
    void:   ['.L......L...', 'LLm....mLL..', '..LmmmmmmL..']
  };
  ...
  function addCrest(canvas, style, material) {
    ...
    /* Crest data is authored at 1x, so each of its pixels is stamped as a
       d x d block to sit correctly on a hero baked at d detail. */
    const d = canvas.detail || 1;
    const m = Armor.MATERIALS[material];
    const out = A.makeCanvas(canvas.width, canvas.height + rows.length * d, d);
    const cx = out.getContext('2d');
    cx.drawImage(canvas, 0, rows.length * d);

    for (let y = 0; y < rows.length; y++) {
      const row = rows[y];
      for (let x = 0; x < row.length && x * d < out.width; x++) {
        const ch = row[x];
        if (ch === '.') continue;
        cx.fillStyle = ch === 'L' ? m.light : m.mid;
        cx.fillRect(x * d, y * d, d, d);
      }
    }
    return out;
  }
  ...
  function buildSet(armor) {
    ...
    function frame(rows) {
      let spr = A.makeSprite(rows, pal, DS.SPR.heroDetail || 1);
    ...
    set.flip = {
      ...
      attack: set.attack.map(A.flipped),

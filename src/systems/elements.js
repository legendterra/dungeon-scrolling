...
(function (DS) {
  ...
  function baseEffect(g, e, element, power) {
    ...
    if (element === 'fire') {
      ...
      DS.FX.element('earth', DS.Ent.centerX(e), e.y + e.h, { count: 10, power: 1.3 });
  ...
  function freeze(g, e, frames) {
    ...
    DS.FX.element('ice', DS.Ent.centerX(e), DS.Ent.centerY(e), { count: 8, power: 1.2 });
  }
  ...
  function updateFields(g) {
    ...
    for (let i = g.fields.length - 1; i >= 0; i--) {
      ...
      } else if (f.element === 'fire' && g.frames % 3 === 0) {
        // A patch of fire should look like flames, not like rising dots.
        DS.FX.element('fire', f.x + DS.rand.float(-f.r, f.r), f.y - 2,
                      { count: 1, power: 0.8 });
      } else if (f.element === 'leaf' && g.frames % 10 === 0) {
        DS.FX.element('leaf', f.x + DS.rand.float(-f.r, f.r), f.y - 6,
                      { count: 1, power: 0.6 });
      } else if (f.element === 'water' && g.frames % 12 === 0) {
        DS.FX.element('water', f.x + DS.rand.float(-f.r, f.r), f.y - 2,
                      { count: 1, power: 0.5 });
      } else if (f.element === 'ice' && g.frames % 14 === 0) {
        DS.FX.element('ice', f.x + DS.rand.float(-f.r, f.r), f.y - 2,
                      { count: 1, power: 0.4 });
      }
  ...
  function tick(g, e) {
    ...
    /* Status you can SEE: a burning/chilled/shocked enemy drags a small patch
       of its element along the ground under it. In voxel mode that patch is a
       glowing 3D plate (R3D), in 2D the flat band already drawn by drawFields
       for real fields — here we only own the per-enemy puddle. */
    const seen = ['burn', 'poison', 'chill', 'shock', 'root'];
    let shown = null;
    for (let i = 0; i < seen.length; i++) if (s[seen[i]] > 0) { shown = seen[i]; break; }
    if (!shown && s.frozen > 0) shown = 'ice';
    const shownEl = shown === 'burn' ? 'fire'
                  : shown === 'chill' || shown === 'frozen' ? 'ice'
                  : shown === 'shock' ? 'lightning'
                  : shown;   // poison/root -> poison/leaf-ish ground colours
    if (shownEl) {
      if (!e.elemPuddle || e.elemPuddle.el !== shownEl) {
        if (e.elemPuddle && DS.R3D.removeElemPuddle) DS.R3D.removeElemPuddle(e.elemPuddle.mesh);
        e.elemPuddle = {
          el: shownEl,
          mesh: DS.R3D.spawnElemPuddle ? DS.R3D.spawnElemPuddle(shownEl, DS.Ent.centerX(e), e.y + e.h) : null
        };
      } else if (DS.R3D.updateElemPuddle) {
        DS.R3D.updateElemPuddle(e.elemPuddle.mesh, DS.Ent.centerX(e), e.y + e.h);
      }
    } else if (e.elemPuddle) {
      if (DS.R3D.removeElemPuddle) DS.R3D.removeElemPuddle(e.elemPuddle.mesh);
      e.elemPuddle = null;
    }
